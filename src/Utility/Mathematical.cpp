...
std::vector<int> RandIntArray(int lb,int ub, int lng, int rpt)
{
    initrand();
    int cnt=0,num;
    std::vector<int> ary; ary.reserve(lng);
    for(; cnt<lng; cnt++)
    {
        if(!rpt)
        {
...
int ChkNumInAry(int num,int lng, std::vector<int> ary)   // check whether num exists in the array of length lng with array base address ary
{
    int cnt=0,flag=0;
    for(; cnt<lng; cnt++)
    {
